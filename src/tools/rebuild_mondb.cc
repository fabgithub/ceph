...
int update_osdmap(ObjectStore& fs, OSDSuperblock& sb, MonitorDBStore& ms)
{
  ...
  if (sb.oldest_map > first_committed) {
    t->put(prefix, first_committed_name, sb.oldest_map);
  }

  // add inc maps
  unsigned nadded = 0;
  const auto last = max(last_committed+1, sb.oldest_map);
  for (auto e = last; e <= sb.newest_map; e++) {
  ...
  if (sb.newest_map > last_committed) {
    t->put(prefix, last_committed_name, sb.newest_map);
    t->put(prefix, ms.combine_strings("full", "latest"), sb.newest_map);
  }
  ms.apply_transaction(t);
  string osd_name("osd.");
